...
# If enabled, this new option means that if you whitelist a network, that only people in one of your channels on
# that network will be able to PM you.  If it's disabled (the default), the old behaviour happens, anyone on 
# that network (in one of your channels or not) will be able to PM you.
### 1.1: David O'Rourke
# Made a small function named unique() which checks that entries in an array are unique.
# Modified various parts of the script to use the new unique() function.
# Fixed the check function so that it doesn't modify the array it's checking. (It was lc()ing things).
# Removed some old commented out code.  Added a few comments to various things.
### 1.0: David O'Rourke
